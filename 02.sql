CREATE TABLE IF NOT EXISTS aoc2020_02
(
    min INT,
    max INT,
    ch  CHAR,
...
INSERT INTO aoc2020_02
VALUES (8, 9, 'x', 'xxxxxxxrk'),
       (5, 8, 'f', 'fxffmfff'),
       (2, 5, 'c', 'ccspwpc'),
       (1, 4, 'g', 'gggg'),
       (9, 10, 'l', 'lllllllllk'),
...
SELECT COUNT(occurences) AS valid_passwords
FROM (SELECT array_length(string_to_array(pw, ch), 1) - 1 AS occurences, min, max FROM aoc2020_02) AS o
WHERE o.occurences >= o.min
  AND occurences <= o.max;

SELECT COUNT(occurences) AS valid_passwords
FROM (SELECT ch, substring(pw, min, 1) AS ch1, substring(pw, max, 1) AS ch2 FROM aoc2020_02) AS occurences
WHERE (ch1 = ch AND ch2 != ch)
   OR (ch1 != ch AND ch2 = ch);
